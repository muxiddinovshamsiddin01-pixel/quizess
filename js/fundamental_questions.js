...
window.FUNDAMENTAL_QUESTIONS = [
  ...
  // ── Q1: Homogeneity (Open — written answer) ───────────────────────────
  {
    id: 1,
  ...
  // ── Q3: Poisson Coefficient (MCQ) ────────────────────────────────────
  {
    ...
    opts: [
      ...
      'The ratio between **axial** and **transverse** deformation measured on a specimen during monotonic tensile testing. Its units are **MPa**',
      'The ratio between **axial** and **transverse** deformation measured on a specimen during monotonic tensile testing. Its units are dimensionless',
    ],
    correct: 1,
  ...
  // ── Q5: Internal actions at worst point (Open) ────────────────────────
  {
    ...
    type: 'open',
    q: 'From the following structure, **determine the internal actions at the worst point** and indicate the **coordinate of the worst point** according to the red coordinate system (origin at A, x→ right).\n\n![Beam with coordinate system, fixed at A (pin), roller at B — distributed load on right span](images/fundamental/q5_beam.png)',
    answerTemplate: 'Coordinate of the worst point    x = [   ]\nBending moment at worst point    M = [   ]\nNormal force at worst point      N = [   ]\nShear force at worst point       V = [   ]\nTorque at worst point            T = [   ]',
    explanation: '## Solution — Finding the Worst Point\n\n**Reactions (from Q4):** V_A = 62.5 N, V_B = 37.5 N, H_A = 0, M_A = 150 N·m\n\n### Shear Force Diagram (V(x)):\n\n**Span 1: 0 ≤ x ≤ l₁ = 2 m** (no distributed load)\nV(x) = V_A = +62.5 N (constant)\n\n**Span 2: l₁ ≤ x ≤ l₁+l₂ = 4 m** (UDL q = 50 N/m ↓)\nV(x) = 62.5 − 50·(x − 2)\n\nV = 0 when: x = 3.25 m\n\n### Bending Moment Diagram (M(x)):\n\nAt x=0: M = −150 N·m | At x=2: M = −25 N·m | At x=3.25: M = +14.06 N·m\n\nThe **worst (most critical) section** is at the **fixed support A, x = 0** (largest |M|):\n\n| Internal Action | Value |\n|---|---|\n| **x (coordinate)** | **x = 0 m** |\n| **Bending Moment M** | **−150 N·m** |\n| **Normal Force N** | **0 N** |\n| **Shear Force V** | **+62.5 N** |\n| **Torque T** | **0 N·m** |\n\n*For a straight beam in a single plane with transverse loading only, N = 0 and T = 0 throughout.*',
  },
